...
from decorators import iterate, optional, forall
...
def add_materials(filepath=None, link=True):
    ...
    assert os.path.splitext(filepath)[1] == '.blend', 'A blender file should be given for the textures'
...
def set_materials(materials, objs):
    ...
    for obj in objs:
        ...
        if any([x in obj.name for x in ['corniche', 'traverse']]):
...
def rename_objects_from_material(objects):
    ...
    cat_desc = []
    for obj in objects:
        match = re.search('material_(Label_([a-z]+)_Name_)?',
...
def generate_renders(objects=None, n_frames=1, min_coverage=0):
    ...
    decks = [obj for obj in bpy.data.objects if 'piedroit' in obj.name]

...
def verify_format(ext):
    # the . needs to be included in ext string, e.g. '.json'
    def inner_verify(func):
        @functools.wraps(func)
        def wrapper_verify(*args, **kwargs):
            if os.path.splitext(args[0])[1]!=ext:
                sys.exit(f'{ext} file is expected as input, {os.path.splitext(args[0])[1]} given.\n')
            else:

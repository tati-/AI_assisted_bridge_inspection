...
@forall
def translate_keys(dictio: dict, word2word: dict=word2word) -> dict:
    """
    translates the keys each dictionary in a list based on the word to word translation
    of word_trans
    """
    d = flatdict.FlatDict(dictio, delimiter='.')
    for word in word2word.keys():
        pattern = f'(^|\.){word}(\.|$)'
        for key in d.keys():
            new_key = re.sub(pattern, rf'\1{word2word[word]}\2', key)
            d[new_key] = d.pop(key)

    return d.as_dict()


def translate_string(string: str, word2word: dict=word2word, delimiter='"') -> str:
    """
    ...
    for word in word2word.keys():
        ...
        string = re.sub(pattern, rf'\1{word2word[word]}\2', string)
    return string


@verify_format('.json')
def translate_json(json_file: str, word2word: dict=word2word, case_sensitive: bool=False):
    """
    ...
    if case_sensitive:
        ...
        string = translate_string(string.lower())

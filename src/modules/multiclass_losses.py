"""
implementation of multiclass losses in keras tensorflow, taken from
...
def multiclass_weighted_tanimoto_loss(class_weights: Union[list, np.ndarray, tf.Tensor]=None) -> Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    """
    ...
    """
    if not isinstance(class_weights, tf.Tensor) and class_weights is not None:
...
def multiclass_weighted_dice_loss(class_weights: Union[list, np.ndarray, tf.Tensor]=None) -> Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    """
    Weighted Dice loss.
    Used as loss function for multi-class image segmentation with one-hot encoded masks.
    :param class_weights: Class weight coefficients (Union[list, np.ndarray, tf.Tensor], len=<N_CLASSES>)
...
def multiclass_focal_loss(class_weights: Union[list, np.ndarray, tf.Tensor],
                          gamma: Union[list, np.ndarray, tf.Tensor] = [0]) -> Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    """
    Focal loss.
